...
// --------------------------------------- property notes parsed into structured rules

export interface PropertyNoteRule {
  property_code: string
  kind:
    | 'group_lead_time'
    | 'hard_blackout'
    | 'seasonal_discount_cap'
    | 'required_document'
    | 'overflow_referral'
    | 'seasonal_rate_note'
    | 'operational_note'
  human_rule: string
  /** Structured detail the group tools consume. Concierge only surfaces it. */
  detail: Record<string, unknown>
}

/**
 * The seasonal caps and routing rules hide in free-text `notes` on the properties
 * export. Parsed here so no runtime model has to notice them.
 */
export const PROPERTY_NOTE_RULES: PropertyNoteRule[] = [
  {
    property_code: 'SOL-CHI',
    kind: 'group_lead_time',
    human_rule: 'Group requests over 25 rooms need at least 2 weeks of lead time.',
    detail: { over_rooms: 25, min_lead_days: 14 },
  },
  {
    property_code: 'SOL-AUS',
    kind: 'hard_blackout',
    human_rule: 'SXSW week is a hard blackout. No group holds are accepted regardless of discount.',
    detail: { start: '2027-03-10', end: '2027-03-19', no_group_holds: true },
  },
  {
    property_code: 'SOL-DEN',
    kind: 'seasonal_discount_cap',
    human_rule: 'Ski-season weekends (December to February, Thursday through Sunday) cap the group discount at 8%.',
    detail: { months: [12, 1, 2], weekdays: [4, 5, 6, 0], cap_pct: 8 },
  },
  {
    property_code: 'SOL-SAC',
    kind: 'seasonal_discount_cap',
    human_rule: 'State-legislature session weeks (January to May, weekdays) cap the group discount at 8%.',
    detail: { months: [1, 2, 3, 4, 5], weekdays: [1, 2, 3, 4, 5], cap_pct: 8 },
  },
  {
    property_code: 'SOL-CMH',
    kind: 'required_document',
    human_rule: 'Youth groups require an insurance certificate on file before the block is confirmed.',
    detail: { document: 'insurance certificate', applies_to: 'youth groups' },
  },
  {
    property_code: 'SOL-PVD',
    kind: 'overflow_referral',
    human_rule:
      'Blocks over 15 rooms should be routed to the Boston-area sister property. That property is NOT in our directory, so refer without quoting inventory, rates or availability for it.',
    detail: { over_rooms: 15, referral_target: 'Boston-area sister property', target_in_directory: false },
  },
  {
    property_code: 'SOL-PHX',
    kind: 'seasonal_rate_note',
    human_rule:
      'Rates run roughly 20% lower June to August (off-peak). Directional only: never compute a quoted rate from it.',
    detail: { months: [6, 7, 8], direction: 'lower', approximate_pct: 20, quotable: false },
  },
  {
    property_code: 'SOL-TPA',
    kind: 'operational_note',
    human_rule: 'Popular for sports-team travel; has a dedicated group check-in lane.',
    detail: {},
  },
  {
    property_code: 'SOL-NSH',
    kind: 'operational_note',
    human_rule: 'Very high leisure demand at weekends; weekday group blocks are preferred.',
    detail: {},
  },
  {
    property_code: 'SOL-CLT',
    kind: 'operational_note',
    human_rule: 'New property, opened 2025; still building a group-sales track record.',
    detail: {},
  },
]
