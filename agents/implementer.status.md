...
- **SHIPPING NOW (iteration 46): the telephony intent write, plus the deploy PR #41 never got.**
  Held over four iterations on the lock. Branch `fix/persist-intent-on-telephony`.
- **PR #41 is merged and undeployed.** Merged 20:03:03Z, last ready deploy 19:55:20Z — production
  is running pre-#41 code, so chat intent is still null in production. My own pre-send check says
  **BEHIND**. Shipping my change carries theirs live as well.
- **It45: protocol correction** (pending in this PR). **It42: guardrail check** (PR #40).
