...
- **Measured against production, every link:** all three documented addresses exist as auth users, emails
  confirmed, profile roles `concierge`/`group_sales`/`admin`, **all three sign in** with the password
  `seed-users.mjs` sets, and each session **reads its own role back through RLS with the anon key**. Three
  auth users, three profile rows, no extras. **No defect.**
- **The unguarded link:** the addresses that *exist* are whatever `seed-users.mjs` creates; the addresses a
  reviewer is *told* to type live in SUBMISSION.md twice plus three other documents, and **nothing tied them
  together.** Rename one and a reviewer gets *“invalid credentials”* on first contact with the live app.
- **Six cases:** no document may name an unseeded address; **both** lists in SUBMISSION.md must carry every
  seeded login; the two lists must agree; seeded roles must exist in the `staff_role` enum; all three roles
  must be covered (two concierges and no group_sales would pass everything else). Scoped to prose —
  `Login.tsx`'s `you@` and `AdminHome.tsx`'s `name@` are form placeholders, checked before being dismissed.
  `DEMO_LOGINS.md` read when present, skipped when absent, because requiring a gitignored file is It152's bug.
- **The red-check found a hole in my own guard, and it was the case that mattered.** Five mutations; **the
  third passed**: I had written `read('SUBMISSION.md').includes(email)` — the whole file — so deleting
  `sales@` from the credentials block passed on the email template sixty lines below. **It145's defect, ten
  iterations after I logged it.** Now sliced per region: credentials block loses one → **2 failed**, email
  template loses one → **2 failed**.
- **Then the region extraction was wrong too:** `search(/
```.../)` returns where the fence **begins**, so
  the slice held the heading alone. Failed loudly rather than passing on nothing, and fixed by **printing
  both regions and reading them** before trusting either.
- **Three self-inflicted problems in one iteration, all caught** — the file-wide containment by the
  red-check, the broken slice by the first run, two probe artifacts before that.

- **It154 SHIPPED → the commands the documents hand people, and a sentence on the stage beat that was one
  third wrong.** `documented-commands.test.ts` pins every `npm run X` to a real script and **does not look at
  `npx`** — the riskier form, because a missing `npx` tool does not fail, it **downloads**.
- **`npx vite-node` — the live-modification beat — resolves only via `vitest@2.1.9`'s dependency tree.**
  `package.json` never mentions it, and the sentence under the command promises **no network**. A vitest
  upgrade that restructured that would turn the beat into a registry fetch **mid-demo**, with nothing in the
  repo to notice. `npx netlify` is the same shape handled right: SUBMISSION.md says it is deliberately not a
  dependency and the first run takes minutes — which gave the guard its rule: **installed, or say you are
  not.**
- **The third claim in that sentence was false.** Timed it instead of trusting it: **1334ms, 2213ms,
  1252ms** — never under a second, once over two. A presenter repeating *“under a second”* gets contradicted
  by the screen on the highest-attention beat, for a detail nobody minds. The page now gives the measured
  range and says not to promise it is instant. Swept the same overclaim across all twelve docs: **exactly one
  occurrence**, the one I fixed.
- **Seven cases**, with vitest as a live positive proving the resolver works and netlify keeping the
  **exemption path itself** tested. Red-check: *“under a second”* back **2** · *“instantly”* **2** · measured
  figure deleted **1** · no-network dropped **1** · netlify disclosure removed **2** · restored **7 passed**,
  both docs byte-identical.
- **Caught myself rewriting the defect I removed at It150**: the first draft collected `m[0]` then re-ran
  `matchAll` inside the filter to find the offset by index — two passes correlated positionally, in a new
  file, in a cleverer disguise. Rewritten as one pass on `m.index`, near-miss recorded in the comment.
- **A skipped mutation is not a passing mutation.** My first red-check printed `SKIPPED (anchor did not
  match)` for the netlify disclosure because I guessed its wrapping instead of reading it — and that was the
  **exemption path**, the only case proving the guard is a check and not a hole. Re-anchored on text from the
  file: fires 2.
- **A red-check left a deliverable mutated.** The Python retry died on a `cp1252` decode error between
  writing the mutation and restoring it, so `SUBMISSION.md` sat modified. Restored from the snapshot,
  confirmed byte-identical to snapshot **and HEAD**, then **moved the run and restore out of Python into
  separate bash steps** so no interpreter failure can leave a deliverable edited.

- **It153 SHIPPED → nothing open again, so I audited what a reviewer clicks and what they check.**
- **Links are clean.** Every markdown link and bare URL across README, SUBMISSION, AGENTS and all ten
  `docs/*.md`: **18 relative links, 0 broken; 6 external URLs, all live, checked unauthenticated.** The one
  that matters — the GitHub repo in SUBMISSION.md — answers **200 to an anonymous visitor**, so the
  repository a reviewer is handed is genuinely public. `/api/chat` 405 on GET and `/api/group/proposals` 401
  with no token are the documented behaviours, not faults. Nothing to fix.
- **The gap: `repo-floors.test.ts` floors eight README claims and not the test count** — the number stated
  **three times** and the one the README explicitly invites checking (*“`npx vitest run` for the live
  number”*).
- **It cannot be floored, and the measurement is why.** A count is the result of running the suite, and a
  suite cannot run itself to count. Static declaration sites give a **lower** bound because `it.each`
  expands: **629 sites against 898 tests.** A static guard would **fail the README's true claim of 700**;
  building one would mean lowering an honest floor to satisfy a guard — It149's trap. Not built, and the
  measurement is written into the file so nobody tries.
- **Guarded the half that rots**, five cases: the claim must stay a **floor** (nothing stopped `903 tests`
  replacing it — the existing ban reads one paragraph and only rejects *about/roughly*); the **three copies
  must agree**; it must **not undersell** what a static count already shows (one-directional, sound, 71
  sites of headroom today); the **command must stay beside it**; and anti-vacuity.
- **Red-check, each mutation alone, each case naming itself:** different number **1** · exact count **2** ·
  approximation **1** · floor lowered to 400 **1** · command removed **1** · claim deleted **4** · restored
  **23 passed**, README byte-identical. Lowering the floor to 400 fires **only** the understatement case,
  which is how I know it does independent work.
- **Instrument:** my first enumeration used `grep -oE ".{28}..."` and found **two** of the three claims —
  line 202 has fewer than 28 characters before it. **A measurement artifact that looked exactly like a
  finding**; I would have guarded two. Caught by re-measuring without the prefix — It152's *print the
  population before the verdict*, one iteration later.

- **It152 SHIPPED → nothing was open, so I tested the claim a reviewer meets first — and it was false.**
  The plan asserted at 06:38 that the suite passes from a `git clone` **or a Download ZIP**. Extracted what
  actually ships (`git archive HEAD | tar -x`, no `.git`), ran `npm ci` and the suite:
  **1 failed | 892 passed | 5 skipped.**
- **`defect-disclosure-list.test.ts` failed with `fatal: not a git repository`** — not an assertion, a
  collection error, so it read like the package being broken rather than a missing `.git`. `README.md`
  invites a reviewer to run `npx vitest run`; a ZIP reviewer's first action was red.
- **It is my own test and the rule against it is my own rule.** `agents/README.md` has said since It138: *do
  not shell out to git from a test without a fallback*. It137 built `shippedFiles()` for exactly this and
  three guards use it. I wrote this file at **It145** with a bare `execFileSync('git', ['ls-files', '-z'])`.
  **A rule in a file I own did not reach a file I wrote seven iterations later.**
- **Swept before fixing:** three test files invoke git; `shippedFiles.ts` is the fallback and
  `suite-integrity` guards its call with `if (!inAClone)`. Mine was the only unguarded one.
- **The fix is the shared helper, and the proof is the same tree:** `before 1 failed | 892 passed` →
  **`after 893 passed | 5 skipped | 0 failed`**, nothing else changed. The strongest red-check available —
  no mutation needed. Also confirmed not vacuous there: the walk returns **269 files, `fromGit: false`, zero
  `node_modules`**.
- **The 5 skips are It137's deliberate `skipIf(!clone)` cases** (3 in `shipped-files`, 2 in
  `suite-integrity`) — they prove the fallback faithful where both answers exist. Left alone: the skip is
  correct, and rewriting a guard's semantics an hour before submission to recover two cases is a worse trade
  than saying why they skip.
- **Fifth escape casualty in four iterations:** `.split(' ')` in a heredoc arrived as a literal NUL. This
  shell collapses `\` inside quoted heredocs, so ``, ``, ` ` and `
` all arrive as control
  characters. **Rule: `r"""..."""` for any pattern with a backslash, or write the script to a file.**
...
- **The Planner wrote to their own two files mid-ship**, so `git checkout main` aborted after the PR had
  already merged. Handled by the README's own protocol: scratchpad backup of both files, a **separate**
  stash (`planner WIP during It151 ship`), checkout, pull, pop — clean, and **their four older stashes
  untouched**. Content verified identical to the backup; the round-trip only normalised LF to CRLF, and
  `git diff --numstat` shows 38/31 and 21/0 lines, so their next commit stays clean.
